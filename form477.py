...
def convert_cata():
	...
	for year in range(2011, 2016):
		...
		if "countycode" in df.columns:
			df["state"] = df["countycode"].apply(lambda x: int(str(x)[:2]))
			df["county"] = df["countycode"].apply(lambda x: int(str(x)[2:]))
	...
	for year in range(2016, 2019):
		...
		if "ï»¿countycode" in df.columns:
			df["state"] = df["ï»¿countycode"].apply(lambda x: int(str(x)[:2]))
			df["county"] = df["ï»¿countycode"].apply(lambda x: int(str(x)[2:]))
...
fiscal_df.to_excel("TXCountyWifi.xlsx",index=False)

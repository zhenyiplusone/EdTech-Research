...
def read_data():
	...
	for year in range(2000, 2019):
		...
		df = df.loc[((df['State FIPS Code'] == 48) | (df['State FIPS Code'] == "48")) & (df['County FIPS Code'] != 0)][['County FIPS Code', 'Median Household Income']]
		df['Year'] = year
...
def population():
	...
	'''
	raw_df = pd.read_csv('tx.1969_2019.19ages.txt')
	print(raw_df)

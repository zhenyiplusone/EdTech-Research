...
count = pd.read_csv('EarnedIncome/EarnedIncomeTX1819.csv') # can also index sheet by name or fetch all sheets
...
def api_data():
    ...
    ids = [f'LAUCN48{x:03d}0000000003' for x in county_list]

    final_df = pd.DataFrame(columns = ['year', 'period', 'value', 'countyid'])
    for id_break in range(int(len(ids)/20),int(len(ids)/20)+1):
        print(id_break)
        ...
        for series in p['Results']['series']:
            ...
            df['countyid'] = series['seriesID'].split('LAUCN48')[1].split('0000000003')[0]
            final_df = final_df.append(df,ignore_index=True)

    final_df.to_excel("TXCountyUnemploymentRate_1.xlsx",index=False)
...
def fiscal_year():
    ...
    '''
    raw_df = pd.read_excel('TXCountyUnemploymentRateRaw.xlsx')
    ...
    school_year_df.to_excel("TXCountyUnemploymentRateFiscalYears.xlsx",index=False)
